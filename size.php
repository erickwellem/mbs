...
<?php 
	// Page Config
	$strPageName = "Size";
	$strPageIDName = "size_id";
	$strDBTableName = "mbs_sizes";
	$strDBFieldName = "size_name";
	...
	if ($_REQUEST[$strPageIDName])
	{
		...
		if ($_REQUEST['action'] == "delete" && $strPageItemName)
		{
			...
			$strLog = 'Size named "' . $strPageItemName . '" is successfully deleted.';
...
<head>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#frm_size_name").blur(function() { 
				
				var strcheck = $("#frm_size_name").val();
				...
				if (strcheck.length >= 3)
				{
					$("#statusbox").html('<img align="absmiddle" src="<?php echo $STR_URL; ?>img/loading.gif" /> Checking <?php echo $strPageName; ?>...');
						...
						$.ajax({ 
								type: "post", 
								url: "ajax/size_check.php", 
								data: "frm_size_name="+strcheck, 
								success: function(msg) { 
									
									if (msg == "yes") 
									{
										$("#frm_size_name").removeClass("status_not_ok"); 
										$("#frm_size_name").addClass("status_ok");
									...
									{ 
										$("#frm_size_name").removeClass("status_ok"); 
										$("#frm_size_name").addClass("status_not_ok");
				...
				else						
				{
					...
					$("#frm_size_name").removeClass('status_ok'); // if necessary
					$("#frm_size_name").addClass("status_not_ok");
	...
	<div id="content-wrapper" class="row-fluid">
    	...
    	<div id="content" class="span12">
    		...
    		<ul class="breadcrumb">
			  ...
			  <li><a href="size_list.php">Sizes</a> <span class="divider">/</span></li>
			  <li class="active"><?php if ($_REQUEST['action'] == "add") { ?>New<?php } elseif ($_REQUEST['action'] == "edit") { ?>Update<?php } ?> <?php echo $strPageName; ?></li>
			</ul>	

			<!-- Menu -->
			<div class="pull-right">
			<?php if ($admin->getModulePrivilege('sizes', 'add') > 0 && $_REQUEST['action'] == "edit") { ?>
			<a class="btn" href="size.php?action=add" title="New <?php echo $strPageName; ?>"><i class="icon-plus"></i> New</a>
			<?php } ?>
			<?php if ($admin->getModulePrivilege('sizes', 'view') > 0 && $_REQUEST['action'] == "edit") { ?>
			<a class="btn" href="size_view.php?size_id=<?php echo $_REQUEST['size_id']; ?>&action=view" title="View <?php echo $strPageName; ?>"><i class="icon-info-sign"></i> View</a>
			<?php } ?>
			<?php if ($admin->getModulePrivilege('sizes', 'delete') > 0 && $_REQUEST['action'] == "edit") { ?>
			<a class="btn" href="size.php?size_id=<?php echo $_REQUEST['size_id']; ?>&action=delete" onclick="return confirmDeleteSize(this.form)" title="Delete <?php echo $strPageName; ?>"><i class="icon-remove"></i> Delete</a>
			<?php } ?>
			<?php if ($admin->getModulePrivilege('sizes', 'list') > 0) { ?>
			<a class="btn" href="size_list.php" title="<?php echo $strPageName; ?> List"><i class="icon-list-alt"></i> List</a>			
			<?php } ?>
			</div>
			...
			<!-- Form -->
			<form id="frm_size" class="form-horizontal" action="" method="post">
			    ...
			    <div class="control-group">
			      <!-- Size Name -->
			      <label class="control-label" for="frm_size_name">Name</label>
			      <div class="controls">
			        <input type="text" id="frm_size_name" name="frm_size_name" placeholder="Type Size name" class="input-xlarge" value="<?php if ($_REQUEST['frm_size_name']) { echo stripslashes($_REQUEST['frm_size_name']); } elseif (!$_REQUEST['frm_size_name'] && $row['size_name']) { echo stripslashes($row['size_name']); } ?>" data-validation="required" /> *	        
			        <p class="help-block">The size name</p>
			    ...
			    <div class="control-group">
			      <!-- Size Description -->
			      <label class="control-label" for="frm_size_description">Description</label>
			      <div class="controls">
			        <textarea name="frm_size_description" id="frm_size_description" class="input-xlarge" rows="3"><?php if ($_REQUEST['frm_size_description']) { echo stripslashes($_REQUEST['frm_size_description']); } elseif (!$_REQUEST['frm_size_description'] && $row['size_description']) { echo stripslashes($row['size_description']); } ?></textarea>
			        <p class="help-block">The size description</p>
			      </div>
			    </div>
			 
			    <div class="control-group">
			      <!-- Size Active -->
			      <label class="control-label" for="frm_size_active">Activate?</label>
			      <div class="controls">
			        <input type="checkbox" name="frm_size_active" id="frm_size_active" value="yes"<?php if ($_REQUEST['frm_size_active'] == "yes" || $row['size_active'] == "yes") { ?> checked="checked"<?php } ?> /> 
			    ...
			    <div class="control-group">
			      <!-- Button -->
			      <div class="controls">
			        <button class="btn" type="button" name="frm_submit" id="frm_submit"><?php echo $strState; ?> Size</button>
			      </div>
			    </div>
			  </fieldset>
			</form>
			...
			<script>$("#frm_size").validateOnBlur();</script>	
			...
			$(document).ready(function () {
				...
				$("#frm_submit").click(function() { 
					...
					var validate_status = $('#frm_size').validate();

					var straction = $("#action").val();
					<?php if ($_REQUEST['action'] == "edit") { ?>var size_id = $("#size_id").val();<?php } ?>
	    			var frm_size_name = $("#frm_size_name").val();
	      			var frm_size_description = $("#frm_size_description").val();
	      			var frm_size_active = $("#frm_size_active").val();
	      			...
	      			if (validate_status)
	      			{
						...
						"frm_size_name=" + frm_size_name + "&frm_size_description=" + frm_size_description + "&frm_size_active=" + frm_size_active;
			...
			function clearForm()
			{
				// clear
				$("#frm_size_name").val("");
				$("#frm_size_description").val("");	
				$("#frm_size_active").attr("checked", false);
				$("#statusbox").html("");
