...
<head>
<title>Supplier List | <?php echo stripslashes($arrSiteConfig['site_name']); ?></title> 
                        ...
                        <div class="tab-content">
                          <div class="tab-pane active" id="sendtoemail">
								...
								<form id="frm_email" class="form-horizontal" action="" method="post">	
									<fieldset>
										...
										<input type="hidden" name="frm_year_month" id="frm_year_month" value="<?php echo intval($_REQUEST['frm_year_month']); ?>" />
										<input type="hidden" name="frm_store_id" id="frm_store_id" value="<?php echo intval($_REQUEST['frm_store_id']); ?>" />
										<?php if ($_REQUEST['frm_year']) { ?>
										<input type="hidden" name="frm_year" id="frm_year" value="<?php echo intval($_REQUEST['frm_year']); ?>" />
										<?php } ?>
										<input type="hidden" name="report" id="report" value="<?php echo htmlspecialchars($_REQUEST['report']); ?>" />
                          ...
                          <div class="tab-pane" id="preview-sendtoemail">
                            ...
                            <div>
								...
								<?php echo file_get_contents($STR_URL . 'reports_print.php?print=false&pop=yes&'.$_SERVER["QUERY_STRING"].'&action=print');?>
                            </div>
                          </div>
                        </div>
...
<script>
	$(document).ready(function () {
		$(".btn-submit").click(function () {
			...
			var frm_year_month = $("#frm_year_month").val();
			var frm_year = $("#frm_year").val();
			var frm_store_id = $("#frm_store_id").val();
			var report = $("#report").val();

			var dataString = "action=" + action + "&frm_email_to=" + frm_email_to + "&frm_message=" + frm_message + "&frm_send_copy=" + frm_send_copy + "&frm_year_month=" + frm_year_month + "&frm_year=" + frm_year + "&frm_store_id=" + frm_store_id + "&report=" + report;

			var request = $.ajax({							    
								url: "ajax/reports_proc.php",
								type: "post", 
								data: dataString,
