...

{
	...
	*/	
	...
	function resizeImage($strSourceImage, $strDestImage, $strImageType='jpg', $intWidthNew, $intHeightNew)

	{
...

		{
			...
			$rSrcImage = imagecreatefromjpeg($strSourceImage);

		} 

		

		elseif ($strImageType == 'png')

		{

			$rSrcImage = imagecreatefrompng($strSourceImage);

		} 

		elseif ($strImageType == 'gif') 

		{

			$rSrcImage = imagecreatefromgif($strSourceImage);

		}

		

		$intWidthOld = imageSX($rSrcImage);

		$intHeightOld = imageSY($rSrcImage);

		

		// landscape

		if ($intWidthOld > $intHeightOld) 

		{

			$intWidthThumb = $intWidthNew;

			$intHeightThumb = $intHeightOld*($intHeightNew/$intWidthOld);
...

		{
			...
			$intWidthThumb = $intWidthOld*($intWidthNew/$intHeightOld);

			$intHeightThumb = $intHeightNew;
		...
		$rDestImage = ImageCreateTrueColor($intWidthThumb, $intHeightThumb);

		imageCopyResampled($rDestImage, $rSrcImage, 0,0,0,0, $intWidthThumb, $intHeightThumb, $intWidthOld, $intHeightOld); 
...

		{
			...
			imagejpeg($rDestImage, $strDestImage); 
...

		{
			...
			imagepng($rDestImage, $strDestImage); 
...

		{
			...
			imagegif($rDestImage, $strDestImage);

		}

		imagedestroy($rDestImage); 

		imagedestroy($rSrcImage);
...

	{
		...
		<?php if (basename($_SERVER['SCRIPT_FILENAME']) !== 'user_list_excel.php') { ?>
			...
			<form name="search_user" method="post" action="user_list.php">
				...
				<input type="text" name="frm_search_text" placeholder="Enter text to search" size="40" maxlength="128" value="<?php if ($_REQUEST['frm_search_text']) { echo stripslashes($_REQUEST['frm_search_text']); }  ?>" />
...
********************************************/
...

	{
...

		{
			...
			$query = "SELECT * FROM `mbs_activities` where `year`='$intYear' ORDER BY ";							  		
			...
			$queryTotal = "SELECT COUNT(*) FROM `mbs_activities` where `year`='$intYear'";
		...
		$strResult .= "<a class=\"btn\" href=\"activity_list_print.php?action=print&year=".$intYear."\" target=\"_blank\"><img src=\"" . $STR_URL . "img/print_icon.png\" /> Print</a>";

		$strResult .= "&nbsp;&nbsp;&nbsp;";

		$strResult .= "<a class=\"btn ajax callbacks cboxElement\" href=\"activity_list_email.php?action=email&pop=yes&year=".$intYear."\" title=\"Send Activity Price List to Email\"><img src=\"" . $STR_URL . "img/email_icon.png\" /> Email</a>";
		...
		$strResult .= 
			...
			<div align=\"left\">
				...
				<select name=\"year\" onchange=\"javascript:location.href = this.value;\">

					<option value=\"activity_list.php?year=2013\" ".($intYear==2013?"selected":"").">2013</option>

					<option value=\"activity_list.php?year=2014\" ".($intYear==2014?"selected":"").">2014</option>

					<option value=\"activity_list.php?year=2015\" ".($intYear==2015?"selected":"").">2015</option>

				</select>

			</div>

			<div align=\"right\">" . HTML::showPaging($rowTotal[0], $totalPage, 4, array(array('year', $intYear),

																						 array('frm_search_text', urlencode($_REQUEST['frm_search_text'])), 
			...
			<thead class=\"well\">
				...
				<tr>					
					...
					<th scope=\"col\"><div align=\"center\"><a href=\"" . $_SERVER['PHP_SELF'] . "?year=".$intYear."&page_num=" . intval($_REQUEST['page_num']) . "&frm_search_text=" . urlencode($_REQUEST['frm_search_text']) . "&sortby=activity_name&sortmode=" . $strSortMode . "\">Name</a></div></th>

					<th scope=\"col\"><div align=\"center\"><a href=\"" . $_SERVER['PHP_SELF'] . "?year=".$intYear."&page_num=" . intval($_REQUEST['page_num']) . "&frm_search_text=" . urlencode($_REQUEST['frm_search_text']) . "&sortby=activity_category&sortmode=" . $strSortMode . "\">Category</a></div></th>

					<th scope=\"col\"><div align=\"center\"><a href=\"" . $_SERVER['PHP_SELF'] . "?year=".$intYear."&page_num=" . intval($_REQUEST['page_num']) . "&frm_search_text=" . urlencode($_REQUEST['frm_search_text']) . "&sortby=activity_price&sortmode=" . $strSortMode . "\">Price</a></div></th>										

					<th scope=\"col\"><div align=\"center\">Year</div></th>										

					<th scope=\"col\"><div align=\"center\"><a href=\"" . $_SERVER['PHP_SELF'] . "?year=".$intYear."&page_num=" . intval($_REQUEST['page_num']) . "&frm_search_text=" . urlencode($_REQUEST['frm_search_text']) . "&sortby=activity_store_related&sortmode=" . $strSortMode . "\">Store Related</a></div></th>	
...

				{
					...
					$strResult .= 
						...
						<tr "; if ($no%2 == 0) { $strResult .= "class=\"odd\""; } $strResult .= ">
							...
							<td><div align=\"right\"><strong>" . html_entity_decode(stripslashes($row['year'])) . "</strong></div></td>							
		...
		$strResult .= 
			...
			<tfoot>
				...
				<tr>
					...
					<td colspan=\"4\">" . HTML::showPaging($rowTotal[0], $totalPage, 4, array(array('year',$intYear),array('frm_search_text', urlencode($_REQUEST['frm_search_text'])), 
...

	{
		...
		$strResult .= "<a class=\"btn ajax callbacks cboxElement\" href=\"size_list_email.php?action=email&pop=yes\" title=\"Send Size List to Email\"><img src=\"" . $STR_URL . "img/email_icon.png\" /> Email</a>";
...

	{
		...
		$strResult .= "<a class=\"btn ajax callbacks cboxElement\" href=\"store_list_email.php?action=email&pop=yes\" title=\"Send Store List to Email\"><img src=\"" . $STR_URL . "img/email_icon.png\" /> Email</a>";
...

	{
		...
		$strResult .= "<a class=\"btn ajax callbacks cboxElement btn-popover\" href=\"supplier_list_email.php?action=email&pop=yes\" rel=\"popover\" data-content=\"Send the Supplier List to a certain email\" data-original-title=\"Send Supplier List to Email\" title=\"Send Supplier List to Email\"><img src=\"" . $STR_URL . "img/email_icon.png\" /> Email</a>";
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					...
					<div class="span12" style="text-align:center;margin-top:20px;">
						...
						<a class="btn btn-popover ajax callbacks cboxElement" href="booking_view_email.php?action=email&pop=yes&booking_id=<?php echo $row['booking_id']; ?>&frm_email_to=<?php echo $rowSupplier['supplier_email'];?>" rel="popover" data-content="Send the Booking to a certain email" data-original-title="Email Booking" title="Email Booking"><img src="<?php echo $STR_URL; ?>img/email_icon.png" /> Email</a>
...

	{
...

		{
			...
			$query = "SELECT * FROM `mbs_emails` 

					  WHERE `email_name` LIKE '%" . mysql_real_escape_string($strSearchText) . "%' 

					  OR `email_address` LIKE '%" . mysql_real_escape_string($strSearchText) . "%' 

					  ORDER BY `email_name` ASC

					  LIMIT " . $offset . "," . $TABLE_MAX_ROW_PER_PAGE;

			

			$queryTotal = "SELECT COUNT(*) FROM `mbs_emails` 
...

		{
			...
			$query = "SELECT mbs_emails.email_id, mbs_emails.email_name, mbs_emails.email_address, mbs_emails.email_id, mbs_stores.store_name FROM (mbs_emails left join mbs_stores on mbs_emails.store_id = mbs_stores.store_id) ORDER BY mbs_emails.email_name ASC LIMIT " . $offset . "," . $TABLE_MAX_ROW_PER_PAGE;
		...
		<?php if (basename($_SERVER['SCRIPT_FILENAME']) !== 'email_list_excel.php') { ?>

			<form name="search_email" method="post" action="email_list.php">

				<input type="hidden" name="frm_search_referer" value="<?php echo $_SERVER['PHP_SELF']; ?>" />

				<input type="text" name="frm_search_text" placeholder="Enter text to search" size="40" maxlength="128" value="<?php if ($_REQUEST['frm_search_text']) { echo stripslashes($_REQUEST['frm_search_text']); }  ?>" />

				<input class="btn" type="submit" name="frm_search_submit" value="Search Email" onclick="return validateSearch(this.form)" /><br />
			...
			<div align="right">
				...
				<a class="btn" href="email_add.php" title="New Email"><img src="<?php echo $STR_URL; ?>img/add_icon.png" /> New Email</a> 
		...
		<?php
			...
			<div align=\"center\"><h2>Email List</h2></div>

			<section id=\"table_email_list\">

			<table class=\"table table-bordered table-hover\" summary=\"Email List\">
			...
			<thead>
				...
				<tr>					
					...
					<th scope=\"col\"><div align=\"center\">Store</div></th>									

					<th scope=\"col\"><div align=\"center\">Name</div></th>

					<th scope=\"col\"><div align=\"center\">Email Address</div></th>				

					<th scope=\"col\" width=\"30%\"><div align=\"center\">Edit / Delete</div></th>
		...
		<?php
				...
				$strResult .= "
					...
					<tr "; if ($no%2 == 0) { $strResult .= "class=\"odd\""; } $strResult .= ">
						...
						<td id=\"r" . $row['email_id'] . "\"><div align=\"right\">" . $no . ".</div></td>

						<td><div align=\"left\">" . stripslashes($row['store_name']) . "</div></td>

						<td><div align=\"left\">" . stripslashes($row['email_name']) . "</div></td>

						<td><div align=\"left\"><a href=\"mailto:" . strtolower($row['email_address']) . "\">" . stripslashes(strtolower($row['email_address'])) . "</a></div></td>

						<td><div align=\"center\"><a class=\"btn\" href=\"email_edit.php?email_id=" . $row['email_id'] . "\" title=\"Edit Email\"><img src=\"img/edit_icon.png\" /> Edit</a>&nbsp;&nbsp; <a class=\"btn\" href=\"email_delete.php?email_id=" . $row['email_id'] . "&action=delete\" title=\"Delete Email\" onclick=\"return confirmDeleteUser(this.form)\"><img src=\"img/delete_icon.png\" /> Delete</a> &nbsp;&nbsp;</div></td>
...

	{
		...
		if ($_REQUEST['frm_action'] !== 'add' || 
			...
			!$_REQUEST['frm_store_id'] || 

			!$_REQUEST['frm_email_name'] || 

			!$_REQUEST['frm_email_address'] || 
			...
			?>				
				...
				<input type="hidden" name="frm_email_name" value="<?php echo $_REQUEST['frm_email_name']; ?>">

				<input type="hidden" name="frm_email_address" value="<?php echo $_REQUEST['frm_email_address']; ?>">
	...
	} // validateFormEmailAdd()

	function validateFormEmailEdit()

	{

		if ($_REQUEST['frm_action'] !== 'edit' || 

			!$_REQUEST['frm_email_id'] || 			
			...
			?>				
				...
				<input type="hidden" name="email_id" value="<?php echo $_REQUEST['frm_email_id']; ?>">				

				<input type="hidden" name="frm_store_id" value="<?php echo $_REQUEST['frm_store_id']; ?>">
...

	{
...

		{
			...
			<h2>Email Information &raquo; <?php echo stripslashes($row['email_name']); ?></h2>
			...
			<table cellpadding="5" cellspacing="1" border="0">
			  ...
			  <tr>
			  	...
			  	<?php if ($_SESSION['user']['type'] == 'admin') { ?><a class="btn" href="email_add.php" title="New Email"><img src="<?php echo $STR_URL; ?>img/add_icon.png" /> New Email</a>&nbsp;&nbsp;&nbsp; <a class="btn" href="email_edit.php?email_id=<?php echo $row['email_id']; ?>" title="Edit Email"><img src="<?php echo $STR_URL; ?>img/edit_icon.png" /> Edit</a>&nbsp;&nbsp;&nbsp; <a class="btn" href="email_delete.php?email_id=<?php echo $row['email_id']; ?>&action=delete" title="Delete Email" onclick="return confirmDeleteEmail(this.form)"><img src="<?php echo $STR_URL; ?>img/delete_icon.png" /> Delete</a>&nbsp;&nbsp;&nbsp; <a class="btn" href="email_list.php" title="Email List"><img src="<?php echo $STR_URL; ?>img/list_icon.png" /> List</a><?php } else { echo '&nbsp;'; } ?>			  	
			  ...
			  <tr>
			  	...
			  	<td>: <strong><?php echo stripslashes($row['store_name']); ?></strong></td>
			  ...
			  <tr>
			  	...
			  	<td>: <strong><?php echo stripslashes($row['email_name']); ?></strong></td>
			  ...
			  <tr>
			  	...
			  	<td>: <?php echo stripslashes($row['email_address']); ?></td>
			...
			<ul>				
				...
				<li><strong>Created on:</strong> <?php echo HTML::convertDateTime($row['email_created_date']); ?> by <strong><?php echo stripslashes($row['email_created_by']); ?></strong></li>

				<li><strong>Last modified on:</strong> <?php echo HTML::convertDateTime($row['email_modified_date']); ?> by <strong><?php echo stripslashes($row['email_modified_by']); ?></strong></li>
		...
		<form name="myform" action="<?php if (preg_match("/_exec/", $_SERVER['HTTP_REFERER'])) { if ($_SESSION['user']['type'] == 'admin') { echo "email_list.php"; } else { echo "email_list.php"; } } else { echo $_SERVER['HTTP_REFERER']; } ?>">
